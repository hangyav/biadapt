...
def create_input(input_images, input_labels, batch_size, seed=0, shuffle=False):
    ...
    if input_labels is not None:
        image, label = tf.train.slice_input_producer([input_images, input_labels], shuffle=shuffle, seed=seed)
        return tf.train.batch([image, label], batch_size=batch_size)
    else:
        image = tf.train.slice_input_producer([input_images], shuffle=shuffle, seed=seed)
        return tf.train.batch(image, batch_size=batch_size)


def create_per_class_inputs(image_by_class, n_per_class, class_labels=None, seed=0, shuffle=False):
    ...
    for images, label in zip(image_by_class, class_labels):
        ...
        images, labels = create_input(images, labels, n_per_class, seed=seed, shuffle=shuffle)
...
class SemisupModel(object):
    ...
    def __init__(self, model_func, num_labels, input_shape, test_in=None, test_label=None):
    ...
    def add_accuracy(self, logits, labels):
        # accuracy_v, accuracy = tf.metrics.accuracy(
        #     tf.cast(labels, tf.int64),
        #     tf.argmax(logits, 1))
        correct = tf.equal(
            tf.cast(labels, tf.int64),
            tf.cast(tf.argmax(logits, 1), tf.int64))
        accuracy = tf.reduce_mean(tf.cast(correct, tf.float32))
        accuracy = tf.Print(accuracy, [accuracy], message='train_accuracy: ')
        tf.summary.scalar('Accuracy_Train', accuracy)

    def add_test_accuracy(self):
        # accuracy_v, accuracy = tf.metrics.accuracy(
        #     tf.cast(self.test_label, tf.int64),
        #     tf.argmax(self.test_logit, 1))
        correct = tf.equal(
            tf.cast(self.test_label, tf.int64),
            tf.cast(tf.argmax(self.test_logit, 1), tf.int64))
        accuracy = tf.reduce_mean(tf.cast(correct, tf.float32))
        accuracy = tf.Print(accuracy, [accuracy], message='test_accuracy: ')
    ...
    def create_train_op(self, learning_rate, optimizer=None):
        ...
        if optimizer is None:
            ...
            #data = optimizer.split(';')
            #params = dict([d.split('=') for d in data[1:]])
            #for k,v in params.items():
                #params[k] = float(v)
            trainer = getattr(tf.train, optimizer)(learning_rate)
        else:
